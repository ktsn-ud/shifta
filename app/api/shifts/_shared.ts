...
export const shiftInputSchema = z
  .object({
    ...
    startTime: z.string().regex(TIME_ONLY_REGEX, "HH:MM形式で入力してください").optional(),
    endTime: z.string().regex(TIME_ONLY_REGEX, "HH:MM形式で入力してください").optional(),
...
function validateShiftInput(input: ShiftInput) {
  if (input.shiftType === "LESSON") {
    if (!input.lessonRange) {
      throw new ShiftValidationError("shiftType=LESSON の場合 lessonRange は必須です")
    }
    ...
    if (input.lessonRange.startPeriod > input.lessonRange.endPeriod) {
      throw new ShiftValidationError("startPeriod は endPeriod 以下で指定してください")
    }
  ...
  if (input.lessonRange) {
    throw new ShiftValidationError("shiftType が LESSON 以外の場合 lessonRange は指定できません")
  }
  ...
  if (toMinutes(input.startTime) === toMinutes(input.endTime)) {
    throw new ShiftValidationError("startTime と endTime は同じ時刻にできません")
  }
}
...
export async function buildShiftData(
  ...
  if (input.shiftType === "LESSON") {
    if (workplaceType !== "CRAM_SCHOOL") {
      throw new ShiftValidationError("LESSON は CRAM_SCHOOL 勤務先でのみ登録できます")
    }
    ...
    const lessonTimes = await resolveLessonTimeRange(input.workplaceId, lessonRange)
