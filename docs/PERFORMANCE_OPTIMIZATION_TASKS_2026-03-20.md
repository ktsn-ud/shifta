...
- [x] T1-1 (prefetch 復帰) → 最速で体感改善
- [x] T1-2, T1-3, T1-4, T1-5 (sync 非同期化) → 保存時レイテンシ改善
- [x] T1-6, T1-7 (loading.tsx) → UX体感改善

Phase 2 推奨順序:

- [x] T2-1, T2-2 (auth 最適化) → infrastructure 整備
- [x] T2-3～T2-6 (RSC 化) → waterfall 排除
- [x] T2-7 (no-store 見直し) → fetch 頻度最適化

Phase 3 (要件優先度による):

- [ ] T3-1 (ジョブキュー) → 堅牢性向上（見送り）
- [x] T3-2 (dynamic import) → bundle 最適化
- [ ] T3-3 (計測) → 継続的監視（見送り）
