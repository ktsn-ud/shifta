...
## 移行時点のコード確認結果（2026-04-30）

以下は **2026-04-30時点の旧実装に対する移行確認記録** であり、現行実装の仕様説明ではない。現行の給与計算は本書冒頭のV2定義を正本とし、旧コードの記述や必要変更一覧を未実装事項として解釈しないこと。

本仕様に対して、当時の実装は旧仕様（休日時給差し替え + 割増率が倍率前提）で動いていた。主な移行差分は以下。
...
## 必要変更の洗い出し（2026-04-30時点の移行履歴）

以下は上記時点での移行計画・確認記録であり、現行の未実装一覧ではない。
