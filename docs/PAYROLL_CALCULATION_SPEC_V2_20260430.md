...
- `nightMultiplier`: 深夜割増率
  - 可能ならば、より適切なカラム名に変更する。
  - デフォルト値は 0.25 (法律で決まっている最低値) とする。
  - 基本給に乗ずるときは (1 + 割増率) とする必要があることに注意。
- `overtimeMultiplier`: 所定時間外割増率
  - これも可能ならばより適切なカラム名に変更する。
  - デフォルト値は 0.25
...
1. **残業割増の扱いが仕様本文で不明確**  
   DB項目には `overtimeMultiplier` を残す記述がある一方、支給額分類は「基本/深夜/休日」の3区分のみ。  
   「残業支給を廃止するのか」「3区分外で別途加算するのか」を確定する必要がある。

2. **既存データ移行ルールが未定義**  
   `holidayHourlyWage`（休日時給）から `holidayAllowanceHourly`（加算手当）への変換式が未定義。  
   代表候補: `holidayAllowanceHourly = max(0, holidayHourlyWage - baseHourlyWage)`。

3. **関連設計書との不整合**  
   `docs/DESIGN_SPECIFICATION.md` は現状、旧給与計算仕様を記載。  
   実装前に参照元仕様の優先順位（本V2を正とするか）を明示しないと、実装時に解釈がぶれる。

4. **マイグレーション実行の前提**  
   本変更は Prisma スキーマ変更を伴うため、`prisma migrate` / `prisma generate` 実行タイミングを事前合意する必要がある。
