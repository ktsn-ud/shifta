...
| `/my/shifts/bulk-edit` | SCR_019 | シフト一括編集   | 月内の既存シフトを表形式で一括編集       | 必須     |
...
- `/my` と `/my/shifts/list` の表示月は `month=YYYY-MM` query param を正本とする。未指定または不正値の場合は現在月を表示する。
- `/my/shifts/bulk` の表示月は URL に保持せず、フォーム内部 state で管理する。初期表示・再読み込みは現在月から開始し、月移動で URL は変化しないため、特定月の URL 共有・復元は行わない。
- 月移動は `/my` と `/my/shifts/list` では `router.replace` で query param を更新し、履歴を増やさない。ダッシュボードから一括登録へ遷移するリンクには月 query を付けず、一括登録完了・キャンセル後の `/my?month=...` 復帰ではダッシュボードの表示月を引き継ぐ。
...
- **月表示と編集中状態**: 表示月は URL に保持せずフォーム内部 state で管理する。初期表示・再読み込みは現在月から開始し、前月/翌月への移動はカレンダーの表示月と Google Calendar API の取得対象月だけを変更する。選択日、日付別の編集中行入力、既定値は、選択のリセット、キャンセル、または登録成功まで保持する。
...
**目的**: 指定月の既存シフトを表形式で確認し、編集可能な項目だけをまとめて明示保存する。変更中は支給額への影響を事前に確認できる。

- **URL**: `/my/shifts/bulk-edit?month=YYYY-MM`。`month` は `/my`・シフト一覧・一括登録と同じ月URL規約を使用し、前月/翌月遷移時も URL を更新する。
- **遷移**: サイドバーの「シフト管理」配下に「一括編集」を表示する。パンくずは「ダッシュボード > シフト管理 > シフト一括編集」とする。
- **一覧**: 月内の全シフトを表示し、既定の日付順と勤務先順を切り替えられる。勤務先、日付、種別、確定状態は参照専用で、確定済みシフトも編集可能とする。
- **編集**: `NORMAL` は開始・終了・休憩・交通費・コメント、`LESSON` は時間割セット・開始/終了コマ・交通費・コメントを編集する。LESSON は選択した時間割セット内のコマだけを選択でき、終了コマは開始コマ以降に制限する。時刻・休憩はコマ間の空き時間を含めて即時導出表示する。
- **保存**: 変更行だけを `PATCH /api/shifts/bulk` に送信し、1回あたり最大31件を全行1トランザクションで更新する。1件でも検証に失敗した場合は更新しない。保存中は編集セル・月移動・並び替えを無効化する。成功時は更新DTOで月次クエリを更新し、Google Calendar 同期は保留表示として `after()` でバックグラウンド実行する。
- **支給額への影響プレビュー**: 変更行だけについて保存前後を同一の給与計算・支給月判定・LESSON時間割計算で比較する。支給月別に「変更前」「差分」「変更後」の総支給額を表示し、差分は給与と交通費を分けて示す。入力途中で保存前後のいずれかを計算できない行は差分に含めず未計算件数として表示する。変更を元に戻すか保存成功後は差分を0へ戻し、保存失敗時と並び替え時はドラフトおよびプレビューを維持する。
- **年次表示**: 支給月の年で年次に集計し、既存の実給与優先の年次baselineへ差分を加算する。`ActualPayroll` が存在する勤務先×支給月の給与・交通費差分は年次加算から除外し、実給与のため除外したことを表示する。月次の保存済み概算baselineは実給与で置換しない。
- **レート制限**: `PATCH /api/shifts/bulk` は認証済みユーザーごとに60秒あたり5回までとする。CSRF 検証失敗は枠を消費せず、超過時は `429` と `Retry-After` を返す。
- **未保存警告**: 月移動・ページ離脱時に未保存の変更があれば確認する。
...
- 変更行だけを対象に保存前・保存後の両方を計算し、`差分 = 保存後 - 保存前` とする。
- 支給月別の変更後見込は、保存済み概算baselineに差分を加えて表示する。
- 保存前または保存後のいずれかが未計算の行は差分に含めない。未計算件数と理由を表示し、誤った減額表示を防ぐ。
- 年次は実給与優先baselineを使用する。ただし実給与登録済みの勤務先×支給月の差分は、実給与が正であるため年次への加算対象外とする。
...
一括登録・一括編集の支給額プレビューは、PC/スマホの全ブレークポイントでページ表示時およびページ再訪時に初期最小化する。最小化時も既存仕様の要約（一括登録は追加予定額合計、一括編集は差分合計）を表示し、操作で詳細を開閉できる。開閉操作はキーボードで実行でき、トグルに `aria-expanded` と対応する詳細領域を示す `aria-controls` を設定する。単件の新規・編集プレビューは従来の表示挙動を維持する。

PC:

- 画面右下にフローティング表示する。初期状態は最小化する。
- 登録フォームの送信ボタンや主要入力欄を覆わない位置に配置する。
- 一括登録で支給月が多い場合、プレビュー内を縦スクロール可能にする。最小化時も要約を残し、操作で詳細を展開・折りたたみできる。

スマホ:

- 画面下部にフローティング表示する。
- 初期状態は最小化する。
- 最小化時も要約を表示し、タップで詳細を展開・折りたたみできるようにする。
- 送信ボタンと重なる場合は、フォーム下部に十分な余白を確保する。

共通:

- 開閉状態はページ内でのみ保持し、ページ再訪時は初期最小化へ戻す。

アクセシビリティ:

- フローティング領域はラベル付きの補助領域として扱う。
- 金額更新の読み上げは必要最小限にする。
- 展開・折りたたみボタンは `aria-expanded` と、開閉対象の詳細領域を参照する `aria-controls` を設定する。
- 開閉ボタンはキーボード操作（Enter/Space）で展開・折りたたみできるようにする。
...
`GET /api/payroll/preview-annual` は `data.actualPayrollKeys` として、要求年内の実給与登録済み `{ workplaceId, paymentMonth }` を返す。一括編集の年次差分はこのキーに一致する給与・交通費の差分を加算しない。認証済みユーザーが所有する勤務先の記録だけを対象にする。
...
11. `/my/shifts/bulk-edit` で、時刻・休憩・コマ・交通費の変更に応じて、支給月別の変更前・差分・変更後が表示される。
12. 実給与登録済みの勤務先×支給月は、月次概算表示を保ちつつ年次の編集差分から除外される。
...
| 2026-08-18 00:00:00 +0000 | シフト一括編集を追加 | SCR_019 として月内の既存シフトを表形式で編集・明示保存する導線、API、Google Calendar 背景同期の仕様を追加。 |
