...
- **月移動時の編集中状態**: 前月/翌月への移動はカレンダーの表示月と取得対象月だけを変更する。選択日、日付別の編集中行入力、既定値は、選択のリセット、キャンセル、または登録成功まで保持する。
...
| 日時 | 変更概要 | 具体的な変更内容 |
| 2026-08-26 00:00:00 +0000 | 一括登録の月移動時に編集中状態を保持 | SCR_014 の前月/翌月移動はカレンダー表示月と取得対象月だけを変更し、選択日・日付別の編集中行入力・既定値を、リセット・キャンセル・登録成功まで保持する仕様を追記。 |
